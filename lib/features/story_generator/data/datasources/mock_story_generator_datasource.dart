...
class MockStoryGeneratorDataSource {
  ...
  Future<StoryPrompt> getRandomOptions() async {
    ...
    final scriptureTypes = storyOptions['scripture']!['types'] as List<dynamic>;
    final randomScriptureType =
        scriptureTypes[_random.nextInt(scriptureTypes.length)] as String;

    String randomScripture = randomScriptureType;
    final subtypes =
        storyOptions['scripture']!['subtypes'] as Map<String, dynamic>?;
    if (subtypes != null && subtypes.containsKey(randomScriptureType)) {
      final scriptureSubtypes = subtypes[randomScriptureType] as List<dynamic>;
      randomScripture =
          scriptureSubtypes[_random.nextInt(scriptureSubtypes.length)]
              as String;
    }

    final storyTypes = storyOptions['storyType']!['types'] as List<dynamic>;
    final themes = storyOptions['theme']!['types'] as List<dynamic>;
    final characters = storyOptions['mainCharacter']!['types'] as List<dynamic>;
    final settings = storyOptions['setting']!['types'] as List<dynamic>;

    return StoryPrompt(
      scripture: randomScripture,
      storyType: storyTypes[_random.nextInt(storyTypes.length)] as String,
      theme: themes[_random.nextInt(themes.length)] as String,
      mainCharacter: characters[_random.nextInt(characters.length)] as String,
      setting: settings[_random.nextInt(settings.length)] as String,
    );
  }
